/**
 * \file      face.cpp
 * \author    Bastien Pesamosca & Ankur Mahtani
 * \date      9 Mai 2017
 * \brief     Figure Libre 
 *
...
int main() {
    ...
    /**
 * \brief     Ecriture dans le fichier txt, envoie des commandes sur la MBED puis dans les servo moteurs
  */

     FILE *commande = NULL;
     commande = fopen("/dev/video0","w");
     if(commande == NULL) {
    ...
    /**
 * \param ang_base Commande envoyée sur la base, qui est fixe au sol. Cette commande fait varier les coordonnées de x de la balle dans l'image. 
 * \param ang_mob Commande envoyée au deuxième servo moteur, qui va faire varier les coordonnées en y de notre balle 
 * \brief   Envoie des commandes d'initialisation 
  */

    float ang_base,ang_mob; /// déclaration des deux angles
    ang_base=100.0; /// angle de 100° pour base
    ang_mob=50.0; /// angle de 100° pour mob
    fprintf(commande, "%f,%f\n", ang_base,ang_mob);
        ...
        if (faces.size()!=0){
           ...
           // printf("%lu\n",eyes.size());
            ...
            if ( (abs(pos_x-318)<40) && (abs(pos_y-218)) <40) {
                circle(original, center, 10,CV_RGB(0,255,0),-1,8,0);
            }
            else circle(original, center, 10,CV_RGB(255,0,0),-1,8,0);
            
            char* txt = "UNKNOWN";
            //
            putText(original, txt, text, FONT_HERSHEY_PLAIN, 3.0, CV_RGB(255,0,0), 2.0); /// affichage du statut de l'utilisateur
            /*
            *\brief Calcul de la correction des angles
            */ 
            if (pos_y<238+40) /// Si la balle de ping pong est trop haute 
            {
                ang_mob -= 0.27;
            }
            if (pos_y>238-40) /// Si la balle de ping pong est trop basse
            {  
                ang_mob += 0.27;
            }
            if (pos_x>318+40) /// Si la balle de ping pong est trop à droite
            {      
                ang_base -= 0.27;
            }
            if (pos_x<318-40) /// Si la balle est trop à gauche
            {
            ang_base += 0.27;
            }
